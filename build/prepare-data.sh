...
$MS "$TMP/state/cb_${YEAR}_us_state_500k.shp" \
  ...
  -o format=topojson quantization=1e5 data/states.topo.json
...
$MS "$TMP/county/cb_${YEAR}_us_county_500k.shp" \
  ...
  -simplify "$SIMPLIFY" keep-shapes \
  -o format=topojson quantization=1e5 data/counties.topo.json
...
for fips in $STATES; do
  ...
  $MS "$TMP/place_$fips/cb_${YEAR}_${fips}_place_500k.shp" \
    ...
    -simplify "$SIMPLIFY" keep-shapes \
    -o format=topojson quantization=1e5 "data/places/${fips}.topo.json"
