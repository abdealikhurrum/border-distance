...
region-specific (US: place/county/state; metros: the real local authorities
within ~50 km of the centre, plus the wider region). The 50 km only bounds which
admin polygons are loaded — the units themselves are genuine local-government
boundaries, so a point resolves to its actual borough/authority (Northolt →
Ealing, Watford → Watford, Slough → Slough). Set an optional
...
- A metro's local authorities are the real admin polygons within ~50 km of its
  centre, pulled from official sources (UK: ONS Local Authority Districts,
  spatially queried within 50 km). The wider region comes from OSM via
  `build/prepare-metro.sh`.
